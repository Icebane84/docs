...
def validate():
    ...
    for page in nav_pages:
        if page not in found_files:
             missing_files.append(page)

    if missing_files:
        print(f"❌ [MISSING] The following pages are in docs.json but file missing:", flush=True)
    ...
    if broken_links:
        print(f"⚠️ [BROKEN LINKS] Potential broken internal links detected:", flush=True)
